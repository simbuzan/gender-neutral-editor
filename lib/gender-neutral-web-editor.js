...
export default {
    ...
    toggle() {
        ...
        if (this.rightPanel.isVisible()) { //on toggle open and close rightPanel
            ...
            suggest_button.addEventListener("click", this.findMatch);
        }
    },
                      ...
                      for (var dict_index = 0; dict_index < dictionaries.length; dict_index++) { //iterate over the dictionaries {m: , f: , n:}, {...}, ...
                          for (const [key, value] of Object.entries(dictionaries[dict_index])) { //iterate over k,v pairs m: , f: , n:
                              if (selection === value) {
                                  ...
                                  //neutralized = `<span data-first="${match[0][1]}" data-second="${match[1][1]}" data-third="${match[2][1]}" data-y="${match[2][1]}">' + ${selection} + '</span>`;
                                  masc_form.value = match[0][1];
                                  femn_form.value = match[1][1];
                                  neut_form.value = match[2][1];
                              }
                          }
                      }
                  }
    ...
    insertValues(event) {
        ...
        if (editor = atom.workspace.getActiveTextEditor()) {
            ...
            //check y-alternative format
            let dataThird = "";
            let dataY = "";
            if(neut === "y" || neut === "ys"){
              //dat-third is root+dataY aka a whole word, dataY is a suffix
              dataY = neut;
        ...
        if (selections.length >= 1) {
          ...
          if(beginning === "<span" && isEnd){ //full span
            ...
            matches = [...trimmed.matchAll(/(["'])(?:\\.|[^\\])*?\1/g)];
              ...
              neuter = matches[2][0].replace(/['"]+/g, '').trim();
              if(displayed === ""){
                displayed = neuter;
              }
